...
\subsection{SVM}
In addition to testing recognition algorithms that we implemented ourselves, we used an "off the shelf" SVM method from the sci-kit learn api as a point of reference \cite{sklearn_api}.
...
TODO: DESCRIBE
\footnote{Because of computational constraints, we do not test the VGG-FACE algorithm with DFI.}
...
We tested our algorithms using $3$, $10$, $15$, and $19$ train faces per person to mimic different real-world scenarios. For instance, a system intended to detect the faces of reclusive individuals on a criminal watchlist might only have two or three images of the individual, while a system designed to detect the face of its owner might have ten or twenty images of the individual. \footnote{Because of computational constraints, we tested the VGG-FACE algorithm only with $3$ and $15$ train faces per person.}
...
A deep learning approach to face recognition outperformed the statistical methods of PCA and Sparse Representation and SVM (we find this across all manipulations and parameters). In addition, we find that occlusion decreases recognition accuracy the most while DFI decreases recognition accuracy the least. We include the responses of our algorithms to our manipulations (using $15$ train faces per person) in figure \ref{fig:results}.
...
\caption{Test accuracies with various manipulations.}
\label{fig:results}
\end{figure}
...
\subsection{Equalizing Performance}
In terms of absolute performance, we observe that manipulations tend to decrease recognition accuracy more for algorithms with higher baseline performance \ref{fig:manipulation_impact_absolutedifference}. This mirrors the finding in \cite{russell_super-recognizers:_2009} that inverting images of faces impaired recognition ability more for people with stronger face recognition abilities. However, when we normalize these differences by the baseline accuracy of each algorithm, we no longer see this trend \ref{fig:manipulation_impact_normalizeddifference}.

\begin{figure}[ht]
\centering
\begin{minipage}[c]{0.4\textwidth}
\centering
    \includegraphics[width=2.5in]{../figures/results_plots/manipulation_impact_difference.png}
    \caption{Absolute difference}
    \label{fig:manipulation_impact_absolutedifference}
\end{minipage}
\begin{minipage}[c]{0.4\textwidth}
\centering
    \includegraphics[width=2.5in]{../figures/results_plots/manipulation_impact_normalizeddifference.png}
    \caption{Normalized difference}
    \label{fig:manipulation_impact_normalizeddifference}
\end{minipage}
\end{figure}

\section{Conclusion}
In this project, we implemented and tested a number of recognition algorithms on images subject to a variety of manipulations. In addition to our findings about the performance of the algorithms with various image manipulations we found a trend in which manipulations tended to equalize recognition accuracy between algorithms (similar to a finding in psychology literature that inspired this project), but that this trend disappeared once we normalized the drop in performance on manipulated images by each algorithm's baseline accuracy.
